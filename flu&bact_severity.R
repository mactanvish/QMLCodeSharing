...
# 漏斗图，发表偏倚分析
png(filename = "fp2.png",width = 1500,height = 1500,units = "px",res = 250)
funnel(SeDeath_rma,xlab = "OR",atransf=transf.exp.int,at=log(c(0.1,0.2,0.4,1,2,4,10,20,50)))
dev.off()
...
# 漏斗图，发表偏倚分析
png(filename = "fp3.png",width = 1500,height = 1500,units = "px",res = 250)
funnel(SeICU_rma,xlab = "OR",atransf=transf.exp.int,at=log(c(0.1,0.2,0.4,1,2,4,10,20)))
dev.off()
ranktest(SeICU_rma)
# 敏感性分析,强影响点判断
...
# 漏斗图，发表偏倚分析
png(filename = "fp4.png",width = 1500,height = 1500,units = "px",res = 250)
funnel(SeICU_rma,xlab = "OR",atransf=transf.exp.int,at=log(c(0.1,0.2,0.4,1,2,4,10,20)))
dev.off()
ranktest(SeICU_rma)
# 敏感性分析
...
# 森林图优化(废案) -------------------------------------------------------------------
# death
data <- read_xlsx("森林图优化.xlsx", sheet = "death")
#death主图和亚组图
...
f <- forestplot(labeltext=tabletext, graph.pos=4,
         ...
         xticks= c(0.1,1,5,20),#death time period坐标轴取值
         #xticks = my_xticks,
          graphwidth = unit(0.2,"npc"),#调整刻度的宽度
          xlog = TRUE,# 设置对数坐标轴
         clip = c(0.1,20)#限制显示的坐标轴，可信区间超出部分用箭头表示
...
f
png(filename = "death3.png",width = 2500,height = 1200,units = "px",res = 250)
f
dev.off()

#ICU主图
data <- read_xlsx("森林图优化.xlsx", sheet = "ICU")
...
f <- forestplot(labeltext=tabletext, graph.pos=4,
           ...
           lwd.xaxis=1,#坐标轴线的厚度
           xticks=c(0.1,1,5,20),#death time period坐标轴取值
           graphwidth = unit(0.15,"npc"),#调整刻度的宽度
           xlog = TRUE# 设置对数坐标轴
           ,clip = c(0.1,20)#限制显示的坐标轴，可信区间超出部分用箭头表示
           ,is.summary = c(T,T,F,F,F,F,F,F,F,F,T)
)
f
png(filename = "ICU3.png",width = 3500,height = 1000,units = "px",res = 250)
f
dev.off()

#MV主图
data <- read_xlsx("森林图优化.xlsx", sheet = "MV")
## The rest of the columns in the table.
tabletext <- cbind(c(NA,"Study",data$Study),#Study为顶端label，这样行数+1
...
f <- forestplot(labeltext=tabletext, graph.pos=4,
           ...
           graphwidth = unit(0.15,"npc"),#调整刻度的宽度
           xlog = TRUE# 设置对数坐标轴
           ,clip = c(0.1,32)#限制显示的坐标轴，可信区间超出部分用箭头表示
           ,is.summary = c(T,T,F,F,F,F,F,F,F,F,T)
)
f
png(filename = "MV3.png",width = 3500,height = 1000,units = "px",res = 250)
f
dev.off()

#LOS主图
data <- read_xlsx("森林图优化.xlsx", sheet = "LOS")
## The rest of the columns in the table.
tabletext <- cbind(c(NA,"Study",data$Study),#Study为顶端label，这样行数+1
